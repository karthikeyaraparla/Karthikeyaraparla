### Hey there 👋

Hi, I'm **Karthikeya Raparla**, a **Backend .NET Developer** 🚀 from India.
I love building **scalable APIs, clean backend architectures, and efficient systems**.
Currently open to **new opportunities** and exciting backend-focused roles.

---

🔗 **Connect with me:**

* 💼 LinkedIn: https://www.linkedin.com/in/karthikeya-r/
* 📧 Email: [karthikeyar1811@gmail.com](mailto:karthikeyar1811@gmail.com)
* 🧠 LeetCode: https://leetcode.com/u/karthikeya_sde/
* 📄 Resume: https://drive.google.com/file/d/1ctebF6b6s8jTebFRTrkVtzxhar6inR5x/view?usp=sharing

---

### 👨‍💻 About Me

* 💻 Backend-focused **.NET Developer**
* 🌱 Currently improving **System Design, DSA, and Microservices**
* 🧠 Active on **LeetCode** solving DSA problems
* 🛠️ Love working on **APIs, Databases, and Scalable Systems**
* 💬 Ask me about **C#, .NET, APIs, DSA, SQL**

---

### 🛠️ Tech Stack

**Languages & Frameworks:**

* C#, .NET, ASP.NET Core
* SQL
* Basic Python (for scripting & problem solving)

**Backend & APIs:**

* REST APIs
* ASP.NET Core Web API
* Authentication & Authorization (JWT, etc.)

**Databases:**

* SQL Server
* MySQL
* PostgreSQL

**Tools & Platforms:**

* Git & GitHub
* Visual Studio / VS Code
* Postman
* Docker (Basics)

---

### 📊 LeetCode Stats

![LeetCode Stats](https://leetcard.jacoblin.cool/karthikeya_sde?theme=dark\&font=ABeeZee)

---

### 🚀 What I’m Focusing On

* Writing **clean, maintainable backend code**
* Improving **DSA & problem-solving**
* Learning **Microservices & System Design**
* Building **real-world .NET backend projects**

---

⭐ If you like my work, feel free to check out my repositories and connect with me!
